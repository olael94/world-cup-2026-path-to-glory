...
import { Activity, CheckCircle2, ClipboardList, Eye, Info, RotateCcw, Save, SlidersHorizontal, Trophy, Zap } from "lucide-react";
    ...
    <main className="min-h-screen">
      ...
      {error ? (
        <div className="mx-auto mb-5 max-w-[1400px] px-8">
          <div className="rounded-md border border-coral/40 bg-coral/10 px-4 py-3 text-sm text-coral">{error}</div>
        </div>
      ) : null}
      ...
      <section className="groups-section mx-auto grid max-w-[1400px] gap-5 px-8 md:grid-cols-2 xl:grid-cols-3 2xl:grid-cols-4" data-tutorial="groups">
        {visibleOrders.map((order) => (
          <GroupBoard key={order.group} order={order} result={resultsByGroup.get(order.group)} onDropTeam={onDropTeam} />
        ))}
      </section>

      <MatchScores key={scoreResetVersion} orders={visibleOrders} allOrders={orders} scores={manualScores} onScoreChange={onScoreChange} />
