            ...
            {team && (
                <>
                    ...
                    >
                        ...
                        {/* Header */}
                        <div
                            className="sticky top-0 flex items-center gap-3 px-5 py-4 border-b border-white/8 bg-panel/95 backdrop-blur-sm shrink-0">
                            <div className="shrink-0"><FlagMark team={team} size="lg"/></div>
                            <div className="flex-1 min-w-0">
                                <div className="truncate text-base font-bold text-white">{team.name}</div>
                                <div className="text-xs text-white/50">Group {team.group} · FIFA #{team.ranking}</div>
                            </div>
                            {team.aiAdjusted && (
                                <span
                                    className="hidden shrink-0 rounded-full border border-mint/30 bg-mint/10 px-2 py-0.5 text-[9px] font-semibold uppercase tracking-wider text-mint sm:inline-block">
                  AI Intel
                </span>
                            )}
                            <button
                        ...
                        <div className="flex-1 overflow-y-auto px-5 py-5 space-y-6">
                            ...
                            {team.aiAdjusted ? (
                                <>
                                    {team.intelligenceUpdatedAt && (
                                        <div className="text-[10px] text-white/30 -mt-2">
                                            Last
                                            refreshed {new Date(team.intelligenceUpdatedAt).toLocaleDateString(undefined, {
                                            month: "short",
                                            day: "numeric",
                                            year: "numeric"
                                        })}
                                    ...
                                    {team.intelligenceSummary && (
                                        <p className="text-sm leading-relaxed text-white/80">{team.intelligenceSummary}</p>
                                    )}

                                    {team.newsItems?.length > 0 && (
                                        <Section icon={<Newspaper size={12}/>} title="Latest Intel">
                                    ...
                                    {team.keyPlayersOut?.length > 0 && (
                                        <Section icon={<UserMinus size={12}/>} title="Players Out"
                                            ...
                                            <ul className="space-y-1.5">
                                                {team.keyPlayersOut.map((p, i) => (
                                                    <li key={i}
                                                        ...
                                                        <span
                                                            className="mt-[5px] h-1.5 w-1.5 shrink-0 rounded-full bg-coral"/>{p}
                                                    </li>
                                                ))}
                                            </ul>
                                        </Section>
                                    ...
                                    {team.keyPlayersIn?.length > 0 && (
                                        <Section icon={<UserPlus size={12}/>} title="Players In" titleColor="text-mint">
                                            <ul className="space-y-1.5">
                                                {team.keyPlayersIn.map((p, i) => (
                                                    <li key={i}
                                                        className="flex items-start gap-2 text-xs text-white/70">
                                                        <span
                                                            className="mt-[5px] h-1.5 w-1.5 shrink-0 rounded-full bg-mint"/>{p}
                                                    </li>
                                                ))}
                                            </ul>
                                        </Section>
                                    ...
                                    {team.coachNotes && (
                                        <Section icon={<MessageSquare size={12}/>} title="Coach & Tactics">
                                            <p className="text-xs leading-relaxed text-white/65">{team.coachNotes}</p>
                                        </Section>
                                    )}
                                </>
                            ) : (
                                <div className="rounded-lg border border-white/8 bg-white/5 px-4 py-5 text-center">
                                    ...
                                    <div className="text-xs font-medium text-white/40">AI intelligence not available
                                        yet
                                    </div>
                                    <div className="mt-1 text-[10px] text-white/25">Stats above are based on historical
                                        match records.
                                    </div>
                                </div>
                            )}
    ...
    return (
        <div>
            <div className="mb-1 flex items-center justify-between text-[10px] leading-tight">
                <span className="font-semibold text-white/60">{label} — {note}</span>
